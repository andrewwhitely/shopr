...
function App() {
	const { user } = useAuth0();
	const rawCurrency = user?.user_metadata?.default_currency as Currency | undefined;
	const defaultCurrency =
		rawCurrency && CURRENCIES.includes(rawCurrency) ? rawCurrency : 'USD';
	...
	return (
		<WishlistContext.Provider value={{ itemCount: items.length, totalValue }}>
			<div className='min-h-screen bg-parchment'>
				...
				{/* Main Content */}
				<main className='mobile-container sm:desktop-main-container safe-area-bottom'>
					<ProtectedRoute>
						<div className='sm:desktop-grid'>
							{/* Desktop Sidebar */}
							<aside className='sm:desktop-sidebar hidden sm:block sm:border-r sm:border-warm-stone-200 sm:pr-5'>
